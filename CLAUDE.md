...
**生成では web 検索をしない。** 在庫（`research/themes.md`）から引くだけにする。これが毎日投稿を成立させている唯一の理由である。

### Routines は使わない

**すべてローカルで実行する。** `routines/` は廃止した。
...
日曜に `/note-week` が7本を一括生成する。**生成するのは翌日の月曜から次の日曜まで**である。
日曜に作った有料noteは、その日ではなく**次の日曜**に投稿する。
...
**同じサブカテゴリを連続させない。** 1→2→3 を2周し、日曜の有料noteは
その週に最も厚く書いた領域に寄せる。

日曜の有料noteは、**その週の無料記事のどれかを土台にする。**
無料で「点」を見せ、有料で「線」または「道具」を渡す形にすると、
`letter-audit` の C3（今買う理由）を無料記事の読了直後という文脈で作れる。

### なぜ有料を週2本から週1本に減らしたか

**有料noteは `letter-audit` と `ethics-line` を必ず通す。** ここは削れない（ルール6）。

週2本にすると Opus の呼び出しが倍になるうえ、A1（理想の未来と再現性の交互配置）で
差し戻しが発生したときに、2本分の再判定が同じ週に重なる。実測で2件連続して落ちている。

**1本に絞って、その1本の商品設計に時間を使う。** 無料記事を1本増やしたのは、
無料記事のほうが決定論ゲート（`lint.py` / `dedup.py`）で通せる比率が高く、トークンあたりの
産出が大きいためである。
...
| **`/note-week`** | **日曜に1週間分7本をまとめて作る。主フロー** | **ローカルのみ** |
| `/note-daily` | 記事を1本だけ作る（差し込み・作り直し用） | ローカル |
| `/note-preview` | プレビューを生成し、**ダッシュボードも更新する** | **ローカルのみ** |
| `/note-revise {slug} "指示"` | 差し戻して再生成 | ローカル |
| `/note-letter {slug}` | 有料noteのレター診断ループ | ローカル |
| `/note-review` | ボトルネックを診断して次の1手を1つ決める。**月初に手で回す** | ローカル |
...
**ユーザーが「お願いします」「今週の記事を作って」と言ったら `/note-week` を実行する。**
1本だけと明示されたときだけ `/note-daily` を使う。
...
| 無料記事の `ethics-line` を `lint.py` の一次判定に置換 | Opus 呼び出し 週6 → **ほぼ0** |
| **有料noteを週2本 → 週1本** | **`letter-audit` と `ethics-line` が半分になる** |
...
| 無料記事の執筆（週6本・並列） | Sonnet 5 | `medium` | 量が多い。Opus 5 との品質差が出にくい |
...
**生成はすべてローカルに置く。** Actions が担当するのは LLM を使わない処理だけである。
...
**`routines/` は廃止した。** 生成をすべてローカルに移したため、Routines の定義を持つ理由が消えた。
月次の棚卸し（旧 `monthly-review`）は `/note-review` を月初に手で回すことで代替する。

...
**7本は別々のテーマではない。1つの自動化システムを7つに割った連載である**
（2026-08-21・第3版）。
...
サブカテゴリは ①見せられる自動化 ②自動化フローの設計思想 ③差し替えて使うテンプレート
（`knowledge/account.md` が正本）。**木曜と土曜は必ず①にする。**
UIデザイン（需要÷供給 5.767・1位）タグを毎週使うための条件である。
...
第2版はレターの文章力で C3 を作っており、W33-07 は2周不合格で打ち切っている。
...
notes/{slug}/demo/    before.html / after.html。★サブ①（木曜の画面・土曜のつまずき集）
